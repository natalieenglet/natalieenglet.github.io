...
*/
    ...
    try {
        ...
        // 3. Handle the 'Math.abs' function
        ...
        // 4. Handle trigonometric functions
            ...
            const funcName = func.split('.')[1]; // e.g. "sin"
        ...
        // 5. Handle inverse trigonometric functions
        } else if (
            func === 'Math.asin' || func === 'Math.acos' || func === 'Math.atan'
        ) {
            const funcName = func.split('.')[1]; // "asin", "acos", or "atan"
            history.textContent = `${funcName}(${display.value})`;
            const value = evaluateExpression(display.value);
        ...
        // 8. Handle rounding functions (Math.round, Math.ceil, Math.floor)
            ...
            func === 'Math.round' || func === 'Math.ceil' || func === 'Math.floor'
        ) {
            const funcName = func.split('.')[1]; // "round", "ceil", or "floor"
            history.textContent = `${funcName}(${display.value})`;
            const value = evaluateExpression(display.value);
